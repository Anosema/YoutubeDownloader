...
class downloaderThread(QThread):
	...
	def run(self):
		if self.parent().mode == "Great audio, no video":
			...
			system(f"ffmpeg -y -loglevel quiet -i \"{pathAudio}\" -vn -ab 128k -ar 44100 -y \"" + "\\".join(pathAudio.replace("AudioFolder", "").split("\\")[:-1]) + pathAudio.split("\\")[-1].replace(".webm", "") + ".mp3\"")
			...
			system(f"ffmpeg -y -loglevel quiet -i \"{pathVideo}\" -vn -ab 128k -ar 44100 -y \"" + "\\".join(pathVideo.replace("VideoFolder", "").split("\\")[:-1]) + pathVideo.split("\\")[-1].replace(".webm", "") + ".mp4\"")
			self.tableUpdater.emit((self.x, "Cleaning"))
			...
			system(f"ffmpeg -y -loglevel quiet -i \"{pathVideo}\" -i \"{pathAudio}\" -c:v copy -c:a aac \"" + "\\".join(pathVideo.replace("VideoFolder", "").split("\\")[:-1]) + pathVideo.split("\\")[-1].replace(".webm", "") + ".mp4\"")
			self.tableUpdater.emit((self.x, "Cleaning"))
			self.fileRemover.emit(pathVideo)
			self.fileRemover.emit(pathAudio)
			self.tableUpdater.emit((self.x, "Done"))
